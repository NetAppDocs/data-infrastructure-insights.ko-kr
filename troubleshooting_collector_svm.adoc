...
'''
*문제:* ONTAP SVM 데이터 수집기(워크로드 보안)를 추가하거나 업그레이드한 후, 테넌트 과잉 프로비저닝 또는 구독 위반 경고가 표시됩니다. 새 하드웨어가 추가되지 않았음에도 불구하고 원시 스토리지 용량이 증가했습니다.

*다음과 같이 시도해 보세요:* 동일한 스토리지가 두 번 계산될 수 있습니다. 한 번은 Observability ONTAP 클러스터 수집기에 의해, 또 한 번은 동일한 클러스터의 Workload Security SVM 수집기에 의해 계산됩니다.

Observability ONTAP 컬렉터와 Workload Security SVM 컬렉터가 보고하는 원시 스토리지 용량을 비교하십시오. 각 SVM 컬렉터에 대해 상위 클러스터가 이미 Observability ONTAP 컬렉터에 의해 모니터링되고 있는지 확인하십시오. 이미 모니터링되고 있는 경우 클러스터 용량만 구독 사용량에 포함되어야 합니다.

최근 클러스터 간 SVM 마이그레이션이 발생했는지 확인하십시오. SVM이 마이그레이션된 후 Workload Security가 수집기로부터 업데이트된 정보를 수신할 때까지 용량 메트릭이 이전 클러스터와 연결된 상태로 유지될 수 있습니다.

마이그레이션된 SVM에 대한 ONTAP SVM 데이터 수집기가 RUNNING 상태인지 확인하십시오. 필요한 경우, 마이그레이션 후 Workload Security에서 올바른 클러스터 및 용량 메트릭을 수집할 수 있도록 수집기를 다시 시작하십시오.

클러스터 용량과 SVM 용량이 모두 총계에 표시되는 경우, 용량이 중복 계산되었을 가능성이 높습니다. 이러한 현상은 SVM 컬렉터를 추가, 업그레이드, 재시작하거나 클러스터 간에 SVM을 마이그레이션한 직후에 자주 발생합니다.

사용량이 여전히 구독 한도를 초과하는 경우, 알림 발생 전후의 구독 사용량과 보고된 원시 스토리지 용량이 포함된 Observability 및 SVM 수집기 목록을 첨부하여 NetApp 지원 케이스를 여십시오.

{비어 있는}

'''
*문제*: 데이터 수집기 서비스는 실행 중이지만, 하나 이상의 데이터 수집기가 '실행 중'이 아닌 '성능 저하' 상태로 표시됩니다.

*다음 방법을 시도해 보세요*: SWS는 노드에서 하나 이상의 FPolicy 연결이 끊어지면 "성능 저하"를 보고합니다. 이는 서비스 중단이 아니라(컬렉터는 계속 실행 중) 연결이 복구될 때까지 해당 노드의 감사 이벤트가 손실될 수 있음을 의미합니다.

ONTAP SVM용 운영 데이터 LIF를 보유한 각 노드에서 FPolicy 제어 채널 연결을 설정합니다.  로컬 데이터 LIF가 없는 노드는 연결할 수 없습니다.  참조:

* link:https://docs.netapp.com/us-en/ontap/nas-audit/node-fpolicy-server-communication-process-concept.html["노드와 외부 ONTAP FPolicy 서버 간 통신 프로세스"^]
* link:https://docs.netapp.com/us-en/ontap/nas-audit/fpolicy-external-fpolicy-servers-concept.html["ONTAP FPolicy가 외부 FPolicy 서버와 연동되는 방식"^]
* link:https://docs.netapp.com/us-en/ontap/nas-audit/requirements-best-practices-fpolicy-concept.html["ONTAP FPolicy 구성을 위한 요구 사항, 고려 사항 및 모범 사례"^]


UI에서 연결이 끊어진 노드를 확인한 다음, 해당 노드에 SVM용 데이터 LIF가 있는지 확인합니다.

* 나열된 노드에 데이터 LIF가 없습니다. Degraded 상태를 무시하십시오. 로컬 데이터 LIF가 없는 경우 ONTAP은 "FPolicy 서버에 연결할 로컬 LIF가 없습니다"라는 이유로 FPolicy 연결이 끊어졌다고 표시할 수 있습니다. 이는 정상적인 현상입니다. 을 참조하십시오. link:https://kb.netapp.com/on-prem/ontap/da/NAS/NAS-KBs/Fpolicy_server_not_connected_due_to_no_local_lif_present_on_one_or_more_nodes["KB: Fpolicy 서버가 "로컬 lif가 없음"으로 인해 연결되지 않음"^] 향후 SWS 릴리스에서는 이러한 경우 수집기를 Degraded로 표시하지 않을 예정입니다.
* 나열된 노드에 데이터 LIF가 있습니다. FPolicy 연결이 끊어진 이유(LIF 다운/데이터-fpolicy-client 누락, 네트워크/방화벽 문제 또는 일시적인 재시작/장애 조치)를 조사하십시오.
