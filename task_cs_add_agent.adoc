...
summary: 워크로드 보안 에이전트는 사용자 활동을 모니터링하고 스토리지 환경에서 보안 위협을 감지합니다. 
---
= 워크로드 보안 에이전트 배포
:hardbreaks:
:allow-uri-read: 
...
[role="lead"]
워크로드 보안 에이전트는 사용자 활동을 모니터링하고 스토리지 인프라 전반에서 잠재적인 보안 위협을 감지하는 데 필수적입니다. 이 가이드에서는 사용자 환경에 에이전트를 배포하는 방법에 대한 단계별 지침을 제공합니다. 시작하기 전에 에이전트 서버가 다음 사항을 충족하는지 확인하십시오. link:concept_cs_agent_requirements.html["시스템 요구 사항"].
...
| 고정 및 고정 해제 | 고정 해제는 최신 버전(사용 가능한 경우)을 즉시 가져오고 에이전트와 수집기를 업그레이드합니다. 이 업그레이드 중에 fpolicies의 연결이 끊어지고 다시 연결됩니다. 이 기능은 자동 업그레이드의 타이밍을 제어하려는 고객을 위해 설계되었습니다. 


| 권장 접근 방식 | 대규모 구성의 경우 수집기를 일시 중지하는 대신 고정 및 고정 해제를 사용하는 것이 좋습니다. 고정 해제 및 고정 해제를 사용하는 동안 일시 정지 및 재개할 필요가 없습니다. 고객은 에이전트와 수집가를 고정해 두고, 새 버전에 대한 이메일 알림을 받으면 30일 기간 내에 에이전트를 하나씩 선택적으로 업그레이드할 수 있습니다. 이 접근 방식은 fpolicies에 대한 지연 영향을 최소화하고 업그레이드 프로세스에 대한 더 큰 제어력을 제공합니다. 
|===
