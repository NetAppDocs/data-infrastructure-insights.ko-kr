...
. _admin_ 유형의 vserver 이름을 가져옵니다. 이후 명령에서 이 이름을 사용합니다.
+
...
. 휴식 시간이 지나면 REST 수집기가 계속해서 데이터를 수집함에 따라 데이터가 안정화되는 것을 확인할 수 있습니다.
...
* 처리량 – 내부 볼륨에서 집계됩니다.
* 관리 – 이 항목에는 장치의 관리 인터페이스에 대한 하이퍼링크가 포함될 수 있습니다. Data Infrastructure Insights 데이터 소스에서 인벤토리 보고의 일부로 프로그램적으로 생성됩니다.
