...
[role="lead"]
안전한 토큰 기반 인증으로 보호되는 REST API를 사용하여 Workload Security를 엔터프라이즈 생태계에 통합하세요. 포렌식 활동 데이터를 검색하고, API 액세스 토큰을 관리하고, CMDB, 티켓팅 시스템 및 기타 애플리케이션과의 맞춤형 통합을 개발할 수 있습니다. 대화형 Swagger 문서는 완벽한 API 사양을 제공하며 엔드포인트를 직접 테스트할 수 있도록 지원합니다.
