...
이 컴퓨터에서는 다른 애플리케이션 수준의 소프트웨어를 실행해서는 안 됩니다. 전용 서버를 사용하는 것이 좋습니다.

SELinux를 사용하는 경우 acquisition unit 시스템에서 다음 명령을 실행하는 것이 좋습니다.
...
Linux의 경우 디스크 공간은 다음과 같은 방식으로 할당해야 합니다.

* /opt/netapp 10 GB(대규모 환경의 경우 20 GB)
* /var/log/netapp 40 GB(대규모 환경의 경우 80 GB)
* 설치 중 /tmp에 최소 1GB 사용 가능


파일 시스템은 오버헤드로 인해 약간의 공간을 사용할 수 있습니다. 다음은 설치 전에 _사용 가능해야_ 하는 최소 공간입니다.
| 50GB 
