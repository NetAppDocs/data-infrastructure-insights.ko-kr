...
summary: Workload Security 에이전트는 스토리지 인프라 전반에서 파일 액세스를 효과적으로 모니터링하고 보안 위협을 탐지하기 위해 특정 운영 체제, CPU, 메모리 및 디스크 공간 사양이 필요합니다. 
...
[role="lead"]
최적의 모니터링 및 위협 탐지 성능을 보장하려면 최소 운영 체제, CPU, 메모리 및 디스크 공간 요구 사항을 충족하는 전용 서버에 Workload Security 에이전트를 배포하십시오. 이 가이드에서는 link:task_cs_add_agent.html["Workload Security 에이전트 설치"] 전에 필요한 하드웨어 및 네트워크 요구 사항을 명시하며, 지원되는 Linux 배포판, 네트워크 연결 규칙 및 시스템 크기 조정 지침을 포함합니다.
