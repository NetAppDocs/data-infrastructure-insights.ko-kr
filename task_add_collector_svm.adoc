...
. 언제든지 한 데이터 수집기는 실행 중이어야 하고, 다른 데이터 수집기는 _중지됨_으로 표시되어야 합니다.
+
현재 '실행 중인' SVM의 데이터 수집기는 _실행 중_으로 표시됩니다.  현재 '중지된' SVM의 데이터 수집기는 _중지됨_으로 표시됩니다.

. 전환이 있을 때마다 데이터 수집기의 상태가 '실행 중'에서 '중지됨'으로 변경되고 그 반대의 경우도 마찬가지입니다.
