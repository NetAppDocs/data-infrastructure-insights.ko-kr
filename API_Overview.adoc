...
* ASSETS 유형에는 asset, query 및 search API가 포함됩니다. DII asset 속성 및 metric은 때때로 "odata"라고도 합니다. 이러한 API는 일반적으로 /rest/v1/asset 또는 /rest/v1/query를 통해 액세스할 수 있습니다. 자세한 내용은 <<api-documentation-swagger,API Swagger 문서>>을 참조하십시오.
+
...
* DATA INGESTION 유형은 Telegraf 에이전트와 같은 곳에서 수집 데이터 및 사용자 지정 메트릭을 검색하고 관리하는 데 사용됩니다. 이러한 메트릭은 때때로 "데이터레이크" 또는 "레이크" 메트릭이라고도 합니다. 이러한 API는 일반적으로 /rest/v1/lake를 통해 액세스됩니다. 자세한 내용은 <<api-documentation-swagger,API Swagger 문서>>을(를) 참조하십시오.
...
참고로, 사용자가 접근할 수 있는 API 유형은 각 Data Infrastructure Insights 기능 세트(관찰 가능성, 워크로드 보안, 보고)에서 사용자가 보유한 link:concept_user_roles.html["사용자 역할"]에 따라 달라집니다.
