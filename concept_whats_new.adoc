...
또한, 워크로드 보안 역할 권한 검사의 보안이 한층 더 강화되었습니다.  보안이 강화됨에 따라 일부 기존 사용자는 수집기를 추가할 때 권한 문제를 겪을 수 있습니다.  다음 지침에 따라 사용자 권한을 확인하세요.link:task_add_collector_svm.html#a-note-about-permissions["문서화"] .
...
이제 Data Infrastructure Insights Acquisition Unit과 함께 다음 운영 체제가 지원됩니다link:concept_acquisition_unit_requirements.html["이미 지원됩니다"].
...
=== VM Analyzer: VM-스토리지 간 전체적인 가시성 및 문제 해결

DII의 *VM Analyzer* 기능은 가상화 환경에 대한 포괄적인 엔드 투 엔드 뷰를 제공합니다. VM Analyzer는 가상 머신에서 스토리지까지의 전체 경로를 매핑하여 문제 해결을 간소화하고, 사용자가 가상 디스크, VM, 호스트, 데이터스토어, 볼륨 및 스토리지 전반의 성능 문제를 신속하게 파악할 수 있도록 지원합니다. 직관적인 토폴로지 뷰, 필터링 및 세부적인 성능 지표를 통해 관리자는 병목 현상을 쉽게 파악하고, 트래픽 불균형을 분석하며, 전반적인 스토리지 성능을 최적화할 수 있습니다.
...
=== 새로운 랜딩 페이지 를 통해 흐름을 원활하게 유지할 수 있습니다

새롭게 디자인된 랜딩 페이지는 요약 데이터, 성능 차트, 상관 관계, 경고 및 변경 사항, 주요 리소스 등 필요한 모든 것을 하나의 보기에 표시합니다.

이제 탭 사이를 이동하거나 문맥을 잃지 않고도 문제를 빠르게 해결하고 흐름을 유지할 수 있습니다.

image:lp_new_design.png["요약, 성능 및 추가 리소스 섹션의 배치를 보여주는 새로운 랜딩 페이지 설계"]



=== 워크로드 보안 개선

* 현재 워크로드 보안 경고에 Webhook 사용 가능 *

이제 워크로드 보안은 Slack, PagerDuty, Teams 등의 툴에 대한 즉시 사용 가능한 Webhook 알림을 지원합니다. 또한 메시지를 맞춤화하거나 다른 SIEM 또는 타사 애플리케이션과 통합할 수 있는 사용자 지정 가능한 템플릿을 제공합니다. 중요 알림을 기존 보안 워크플로에 직접 전달하여 팀이 보다 빠르게 조사하고 대응할 수 있도록 합니다.

image:ws_webhook_slack_example.png["워크로드 보안의 Webhook Slack 예, 너비 = 400"]

* 에이전트 간 워크로드 보안 수집기 마이그레이션 *

한 에이전트에서 다른 에이전트로 작업 부하 보안 수집기를 손쉽게 마이그레이션할 수 있으므로 에이전트 간에 수집기의 로드 밸런싱을 효율적으로 수행할 수 있습니다. 컬렉터를 편집하고 목록에서 대상 에이전트를 선택하는 것만큼 쉽게 마이그레이션할 수 있습니다.

image:ws_migrate_collector_to_another_agent.png["Collector 마이그레이션, 폭 = 500"]



=== .csv 비동기 내보내기

데이터를 .csv로 내보내려면 내보낼 데이터 양에 따라 몇 초에서 몇 시간 정도 걸릴 수 있습니다. 이제 Data Infrastructure Insights는 해당 데이터를 비동기식으로 내보내므로 .csv가 컴파일되는 동안에도 작업을 계속할 수 있습니다.

오른쪽 상단 도구 모음에서 "벨" 아이콘을 선택하여 .csv 내보내기를 봅니다.

image:csv_export_async.png["종 아이콘 다운로드 가능한 .csv 내보내기 목록, 너비 = 400"]
