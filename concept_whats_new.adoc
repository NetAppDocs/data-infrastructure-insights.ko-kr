...
=== 새로운 AI Assistant 시작 페이지

DII AI 어시스턴트에 이제 전용 시작 페이지가 추가되어 지능형 인프라 분석을 최우선으로 고려합니다. 새로운 대화를 즉시 시작하거나, 한 번의 클릭으로 마지막 세션을 재개하거나, 제안된 질문을 통해 빠르게 시작할 수 있습니다.

image:AI_dashboard_page_default.png["AI Dashboard 기본, 너비=800"]

탐색 메뉴의 *AI 어시스턴트*에서 시작 페이지에 액세스하거나, 왼쪽 상단의 "별" 아이콘을 사용하여 어디서든 AI 어시스턴트를 열 수 있습니다.

image:AI_dashboard_page_menu.png["AI Assistant 탐색 메뉴, width=600"] image:AI_assistant_access_button.png["어디서든 AI 어시스턴트 이용 가능, width=300"]
...
=== 로그인 메시지를 통해 중요 업데이트 공유

사용자 지정 가능한 로그인 메시지로 팀원들에게 최신 정보를 제공하고 협업을 강화하세요. 중요한 공지사항, 유지보수 일정, 정책 업데이트 등을 사용자가 로그인할 때 직접 전달할 수 있습니다. 마크다운 기능을 활용하여 메시지 형식을 명확하게 지정하고 모든 DII 사용자에게 중요한 정보를 전달할 수 있습니다. 관리자 권한이 있는 계정은 Admin 메뉴의 Notifications에서 구성을 확인할 수 있습니다.
...
DII 탐색 메뉴가 새롭게 단장하여 Console의 색상 및 동작 방식과 일치하도록 개선되었습니다. 기존의 고정된 메인 메뉴는 기본적으로 접혀 있어 작업 공간을 더욱 효율적으로 활용할 수 있습니다. 왼쪽 상단의 햄버거 아이콘(☰)을 클릭하면 메인 메뉴에 접근할 수 있습니다.



[[expand-device-visibility-with-intelligent-ai-assistance]]
=== 지능형 AI 지원으로 디바이스 가시성 확장

AI 기반 장치 식별 기능을 통해 신규 장치 온보딩 속도를 높이세요. Data Infrastructure Insights는 사용자 환경을 분석하여 자동 장치 해결 규칙을 제안함으로써 더 많은 장치를 신속하게 식별하고 관리할 수 있도록 지원합니다. AI가 생성한 규칙을 검토하고 승인하기만 하면 DII가 나머지를 처리하여 인프라 전반에 걸쳐 장치 가시성과 관리를 향상시켜 줍니다.



[[a-smarter-ai-assistant-for-global-teams]]
=== 글로벌 팀을 위한 더욱 스마트한 AI 비서

이제 AI 어시스턴트가 12개 이상의 언어를 지원하여 팀원들이 원하는 언어로 자연스럽게 소통할 수 있도록 지원합니다. 전체 목록을 확인하세요link:https://platform.claude.com/docs/en/build-with-claude/multilingual-support["여기"^].



[[new-monitors-for-infrastructure-attribute-changes]]
=== 인프라 속성 변경을 위한 새로운 모니터

이제 속성 모니터링 기능을 통해 자산이 데이터 센터 간에 이동하거나, 스토리지 풀이 암호화되지 않았거나, 속성이 수정되는 경우 자동으로 팀에 알림을 보낼 수 있습니다. 이를 통해 구성 기준선을 유지하고 새로운 배포 또는 변경 과정에서 누락되는 부분이 없도록 보장할 수 있습니다.

image:attribute_monitor_in_list.png["속성 모니터가 모니터 유형에 추가됩니다"]



[[build-smarter-alerts-and-webhooks-with-dynamic-context-rich-messages]]
=== 동적이고 컨텍스트가 풍부한 메시지를 사용하여 더욱 스마트한 알림 및 웹훅을 구축하세요

모니터 또는 웹훅을 생성할 때 알림 설명이나 웹훅 URL, 헤더 또는 본문에 "%"를 입력하기만 하면 사용 가능한 객체 속성 및 주석 목록에서 동적으로 선택할 수 있습니다. 알림에 적절한 컨텍스트를 제공하여 명확성을 높이고 웹훅 통합을 강화하며 응답 시간을 단축하세요.

image:attribute_list_example.png["모니터 설명의 속성 목록, width=400"]
...
=== Data Infrastructure Insights는 여러분의 환경을 그 어느 때보다 더 잘 이해합니다.

Data Infrastructure Insights의 AI가 사용자 환경을 이해하는 방식을 개선했습니다. 응답하기 전에 서비스의 현재 사용 현황, 구성 및 성능 패턴에 대한 보다 풍부한 컨텍스트를 활용합니다. 그 결과, 일반적인 시작점이 아닌 사용자 환경을 반영한 초기 안내를 제공하여 유용한 답변을 얻기 위한 추가 질문 횟수를 줄일 수 있습니다.

예를 들어, 스토리지 장치로 이동하면 AI Assistant가 다음과 같은 질문에 대한 답변을 한 번의 클릭으로 제공합니다.

* 이 스토리지의 용량 활용률 추세는 어떻고, 어떤 StoragePools가 가장 많은 공간을 차지하고 있습니까?
* 지난 24시간 동안 이 디바이스의 용량 사용률 추세를 보여주시겠습니까?
* 이 스토리지에서 가장 많은 공간을 차지하는 볼륨은 무엇입니까?
...
=== AI Assistant가 이제 정식 출시되었습니다

Data Infrastructure Insights AI Assistant는 이제 AWS에서 호스팅되는 DII 사이트의 모든 고객이 정식으로 이용할 수 있습니다.

Azure 사이트의 AI Assistant는 아직 미리 보기 단계입니다. 정식 출시 발표를 기대해 주세요.



[[infrastructure-insights-features-removed]]
=== Data Infrastructure Insights 기능이 제거되었습니다

DII의 "Infrastructure Insights" 기능 중 _Shared Resources Under Stress_, _Kubernetes Namespaces Running out of Space_, _Reclaiming ONTAP Cold Storage_는 더 이상 DII에서 사용할 수 없습니다.
...
인프라의 새로운 지능형 파트너를 만나보세요: link:ai_for_dii_overview.html["*Data Infrastructure Insights AI Assistant*"]는 복잡한 쿼리를 간단한 대화로 바꿔 데이터와의 상호 작용 방식을 혁신합니다. 숨겨진 패턴을 즉시 발견하고, 문제 해결 속도를 높이며, 데이터 기반 의사 결정을 자신 있게 내릴 수 있습니다. 다음과 같은 질문에 대한 답을 즉시 얻으세요:
...
DII AI 어시스턴트 프리뷰는 현재 AWS 리전에서 서비스를 호스팅하는 대부분의 고객에게 제공되며, Azure 리전은 곧 지원될 예정입니다. Data Infrastructure Insights 미리보기 기능에 대한 자세한 내용은 link:concept_preview_features.html["자세히 알아보기"]를 참조하십시오.

image:ai_assistant_icon.png["AI Assistant 아이콘"]
...
==== 중복 없이 시스템 모니터를 더 빠르게 사용자 지정

기본 시스템 모니터를 필요에 맞게 직접 수정하세요. 이름, 필터, 그룹화를 한 곳에서 변경할 수 있습니다. 간단한 조정을 위해 모니터를 중복 생성할 필요가 없습니다. 유용한 배너를 통해 어떤 모니터가 사용자 지정되었는지 알려주므로 모니터링 구성에 대한 완전한 가시성을 유지할 수 있습니다.



==== 일괄 작업을 통해 모니터 관리 간소화

일괄 작업을 통해 여러 모니터를 한 번에 관리하세요. 모니터 그룹에 대한 상태 모니터링 설정을 신속하게 조정하여 시간을 절약하고 복잡성을 줄일 수 있습니다. 모니터링 전략을 몇 분이 아닌 몇 초 만에 전반적으로 업데이트하세요.



==== 인프라 상태 점수를 가장 중요한 사항에 집중하세요

정확성과 관련성을 유지하기 위해 Infrastructure Health 점수에서 특정 개체를 제외하세요. Infrastructure Health 대시보드를 볼 때 모니터 구성을 수정하지 않고 우선순위를 낮추려는 개체를 즉시 제거할 수 있습니다. 이를 통해 비즈니스 우선순위에 영향을 미치는 인프라 요소에 대한 알림을 집중할 수 있습니다.

image:health_monitoring_exclude_object.png["상태 모니터링에서 객체 제외"]
...
다음 Linux 운영 체제는 현재 Data Infrastructure Insights link:concept_acquisition_unit_requirements.html["Acquisition Unit"]에서 지원됩니다:

* AlmaLinux 9.5 및 10
* Centos(64비트): Stream 9
* Debian(64비트): 11~13
* OpenSUSE Leap 15.2~15.6, 16
* Oracle Enterprise Linux(64비트): 8.10, 9.2, 9.4, 9.6, 10
* Red Hat(64비트): 8.10, 9.2, 9.4, 9.6, 10
* Rocky 9.2~9.6
* SUSE Linux Enterprise Server 15 SP4 ~ 15 SP7, 16
...
Reclaim ONTAP Cold Storage Insight가 이제 FlexGroups를 지원하며, 모든 고객이 이용할 수 있습니다.
...
| Kubernetes 네임스페이스 공간 부족 | _Kubernetes 네임스페이스 공간 부족_ 인사이트는 Kubernetes 네임스페이스에서 공간 부족 위험이 있는 워크로드를 파악하고, 각 공간이 가득 차기까지 남은 예상 일수를 보여줍니다. 


| 스트레스 받는 공유 리소스 | _공유 리소스 스트레스 상황_ 분석 기능은 AI/ML을 사용하여 환경에서 리소스 경합으로 인해 성능 저하가 발생하는 위치를 자동으로 식별하고, 영향을 받는 워크로드를 강조 표시하며, 문제 해결을 위한 권장 조치를 제공하여 성능 문제를 더 빠르게 해결할 수 있도록 지원합니다. 
...
Cloud Insights는 정기적으로 다양한 흥미로운 신규 미리 보기 기능을 소개합니다. 이러한 기능 중 하나 이상을 미리 체험해 보고 싶으시다면 NetApp 영업팀에 문의하여 자세한 정보를 얻으십시오.
...
| 스트레스 받는 공유 리소스 | '스트레스 받는 공유 리소스' 분석 기능은 AI/ML을 사용하여 환경에서 리소스 경합으로 인해 성능 저하가 발생하는 위치를 자동으로 식별하고, 영향을 받는 워크로드를 강조 표시하며, 문제 해결을 위한 권장 조치를 제공하여 성능 문제를 더 빠르게 해결할 수 있도록 지원합니다. 
|===
