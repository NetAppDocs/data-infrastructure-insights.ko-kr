...
[role="lead"]
데이터 수집기를 배포하기 전에 SVM에서 생성되는 NFS 및 SMB 이벤트 발생률을 측정하여 최적의 Agent 시스템 크기를 결정하십시오. Event Rate Checker 스크립트를 사용하면 용량 제한(Agent당 최대 50개의 데이터 수집기)을 파악하고 Agent 인프라가 예상되는 이벤트 볼륨을 처리하여 안정적인 위협 탐지를 수행할 수 있는지 확인할 수 있습니다.
